import { compileOutputSchema, compileRequestSchema } from '@mikage/contracts';
const SECTION_ORDER = [
    'system_frame',
    'canon_constraints',
...
const buildSections = (request) => {
    const contextSummary = request.contextPackets
        .map((packet) => `${packet.packetRef.packetCode}: ${packet.summary} | ${packet.fragments
        .map((fragment) => `${fragment.label}: ${fragment.summary}`)
        .join('; ')}`)
        .join('\n');
    const negativePromptRendered = request.negativePrompt.rendered ||
...
export const compilePrompt = (input) => {
    ...
    return compileOutputSchema.parse({
        ...
        negativePrompt: {
            ...
            rendered: request.negativePrompt.rendered ||
                request.negativePrompt.clauses.join(', ') ||
                'none'
        },
