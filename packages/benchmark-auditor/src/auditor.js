import { benchmarkAuditInputSchema, benchmarkAuditResultSchema } from '@mikage/contracts';
const PASS_ORDER = [
...
const buildSimilarityScore = (label, promptText, referenceTerms) => {
...
export const auditBenchmark = (rawInput) => {
    ...
    const flags = [];
    const goldSimilarity = buildSimilarityScore('gold_similarity', promptText, input.goldBenchmark.referenceTerms);
    const silverSimilarity = buildSimilarityScore('silver_similarity', promptText, input.silverBenchmark.referenceTerms);
    const redTerms = input.redBenchmark.blockedTerms.filter((term) => containsTerm(promptText, term));
    ...
    const riskValue = Number(Math.max(driftScore.score, 1 - goldSimilarity.score, 1 - silverSimilarity.score / 2).toFixed(4));
    const riskScore = {
        score: riskValue,
        riskLevel: riskValue >= 0.75 ? 'high' : riskValue >= 0.35 ? 'medium' : 'low'
    };
    ...
    const decision = redTerms.length > 0 || riskScore.riskLevel === 'high'
        ? {
            decision: 'rejected',
            accepted: false,
