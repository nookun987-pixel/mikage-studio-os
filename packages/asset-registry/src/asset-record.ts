...
export function createAssetRecord(params: CreateAssetRecordParams): AssetRecord {
  return {
...
export function validateAssetRecord(record: AssetRecord): {
  isValid: boolean
  errors: string[]
} {
  const errors: string[] = []
...
export function extractAssetTags(record: AssetRecord): string[] {
  const tags = new Set<string>(record.metadata.tags)
...
export function calculateAssetScore(record: AssetRecord): number {
  if (!record.metadata.evaluation) {
    return 0
  }
...
export function hasAssetIssues(record: AssetRecord): {
  hasWarnings: boolean
  hasErrors: boolean
  issueCount: number
} {
