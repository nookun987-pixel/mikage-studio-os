import { ingestionResultSchema, ingestionInputSchema, persistenceInputSchema, persistenceResultSchema } from '@mikage/contracts';
...
const buildIngestionCode = (requestCode, packageCode) => `ing_${requestCode}_${packageCode}`;
const buildPersistenceCode = (requestCode, ingestionCode) => `persist_${requestCode}_${ingestionCode}`;
const buildSources = (input) => [
    {
...
const buildAssets = (input) => {
    ...
    return [
        ...
        },
        {
            ...
            checksum: buildChecksum(JSON.stringify(productionPackage.validationReference.validation.summary)),
            metadata: input.metadata
        },
        {
...
const buildArtifacts = (input, assets) => {
...
const buildLineageEdges = (input, nodes) => [
    {
...
export const ingestPackageShell = (rawInput) => {
    const input = ingestionInputSchema.parse(rawInput);
    const ingestionCode = buildIngestionCode(input.requestCode, input.packageReference.productionPackage.packageCode);
    const sources = buildSources(input);
    const assets = buildAssets(input);
    const artifacts = localPersistenceAdapter.persistArtifacts(buildArtifacts(input, assets));
...
export const persistLineageShell = (rawInput) => {
    ...
    const persistenceCode = buildPersistenceCode(input.requestCode, input.ingestion.ingestionCode);
