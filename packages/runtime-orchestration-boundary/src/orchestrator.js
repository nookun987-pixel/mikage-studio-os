import { benchmarkAuditInputSchema, compileRequestSchema, ingestionInputSchema, orchestrationRequestAcceptedResponseSchema, persistenceInputSchema, productionPackageInputSchema, studioActionInputSchema } from '@mikage/contracts';
import { auditBenchmark } from '../../benchmark-auditor/src/index.js';
import { validateCanon } from '../../canon-validator/src/index.js';
import { ingestPackageShell, persistLineageShell } from '../../persistence-shell/src/index.js';
...
import { orchestrationChainRequestShellSchema } from './contracts.js';
import { isBenchmarkRejected, isPackagingRejected, isPersistenceRejected, isValidationRejected } from './guards.js';
import { buildExecutedStep, buildRuntimeSummary } from './summary.js';
export const executeGenerationPipeline = (rawInput) => {
    ...
    executedSteps.push(buildExecutedStep('layer3_request_boundary', 'completed', `Accepted orchestration request ${acceptedResponse.boundaryPacket.packetCode}.`));
    const compileRequest = compileRequestSchema.parse({
        ...
        contextPackets: input.request.contextPackets.map((packet, index) => ({
            packetRef: {
                packetKind: packet.packetKind,
                packetCode: packet.packetCode,
                packetVersion: packet.packetVersion ?? 1
            },
            summary: input.compileProfile.contextSummaries[index] ??
            ...
            fragments: [
                {
                    ...
                    summary: input.compileProfile.fragmentSummaries[index] ??
    ...
    const compileMode = input.compileProfile.compileMode;
    executedSteps.push(buildExecutedStep('layer4_prompt_compile', 'completed', `Compiled prompt for ${input.request.presetCode}/${input.request.variantCode}.`));
    ...
    executedSteps.push(buildExecutedStep('layer5_canon_validate', isValidationRejected(validationResult.decision) ? 'stopped' : 'completed', `Validation decision: ${validationResult.decision}.`));
    ...
    const productionPackage = assembleProductionPackage(productionPackageInputSchema.parse({
    ...
    executedSteps.push(buildExecutedStep('layer6_production_package_assembly', isPackagingRejected(productionPackage) ? 'stopped' : 'completed', `Package decision: ${productionPackage.decision.decision}.`));
    ...
    const benchmarkAudit = auditBenchmark(benchmarkAuditInputSchema.parse({
    ...
    executedSteps.push(buildExecutedStep('layer7_benchmark_audit', isBenchmarkRejected(benchmarkAudit) ? 'stopped' : 'completed', `Benchmark decision: ${benchmarkAudit.decision.decision}.`));
    ...
    const ingestionResult = ingestPackageShell(ingestionInputSchema.parse({
    ...
    const persistenceResult = persistLineageShell(persistenceInputSchema.parse({
    ...
    executedSteps.push(buildExecutedStep('layer8_ingestion_and_lineage_persistence', isPersistenceRejected(persistenceResult) ? 'stopped' : 'completed', `Persistence decision: ${persistenceResult.decision.decision}.`));
    ...
    const studioActionResult = runStudioAction(studioActionInputSchema.parse({
    ...
    executedSteps.push(buildExecutedStep('layer9_studio_action_boundary', 'completed', `Studio action projected as ${studioActionResult.normalizedActionType}.`));
    return buildRuntimeSummary({
        requestCode: input.request.requestCode,
        compileMode,
        validationDecision: validationResult.decision,
