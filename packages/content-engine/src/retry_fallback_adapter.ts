 ...
 */
export class RetryFallbackAdapterImpl implements RetryFallbackAdapter {
  ...
  private applyRoutingRules(analysis: any): 'accepted' | 'rejected' | 'retry' | 'fallback' {
  ...
  private generateDecisionConfig(decision: string, analysis: any): any {
    switch (decision) {
      case 'retry':
        return {
  ...
  private isRetryEligible(decision: string, analysis: any): boolean {
    return decision === 'retry' && !analysis.hasTechnicalIssues;
  }

  private isFallbackEligible(decision: string, analysis: any): boolean {
    return decision === 'fallback' || (decision === 'retry' && analysis.qualityScore < 0.6);
  }

  private calculateMaxRetries(decision: string, analysis: any): number {
    if (decision === 'retry') {
      if (analysis.hasTechnicalIssues) return 3;
      if (analysis.hasCanonViolations) return 2;
      if (analysis.qualityScore < 0.8) return 2;
      return 1;
    }
    return 0;
  }

  private hasRetryableConditions(result: ContentEngineResult): boolean {
    ...
    return result.blockingIssues.some(issue => 
      retryableIssues.includes(issue.issueType)
    ) || result.warnings.length > 0;
  }

  private hasFallbackConditions(result: ContentEngineResult): boolean {
    // Check for conditions that suggest fallback
    return (
      result.validationStatus.overall.finalConfidence < 0.7 ||
  ...
  private generateParameterAdjustments(result: ContentEngineResult, type: string): any {
  ...
  private identifySubstituteAssets(result: ContentEngineResult): any[] {
  ...
  private simplifyPrompt(result: ContentEngineResult): string {
  ...
  private selectAlternativeStyle(result: ContentEngineResult): string {
  ...
  private hasCanonViolations(
    ...
    return (
      (preValidationResult.blockingIssues?.some(issue => 
        issue.issueType.includes('canon') || issue.source === 'canon_validator'
      ) || false) ||
      (postValidationResult?.blockingIssues?.some(issue => 
        issue.issueType.includes('canon') || issue.source === 'canon_validator'
      ) || false)
    );
  }

  private hasQualityIssues(postValidationResult: PostGenerationValidationResult | undefined): boolean {
    if (!postValidationResult) return false;
    
    return (
      (postValidationResult.qualityScore !== undefined && postValidationResult.qualityScore < 0.7) ||
      (postValidationResult.issues?.some(issue => issue.type.includes('quality')) || false)
    );
  }

  private selectFallbackStrategy(analysis: any): string {
    if (analysis.qualityScore < 0.5) return 'conservative_generation';
    if (analysis.hasCanonViolations) return 'style_adjustment';
    return 'parameter_reduction';
  }

  private determineRetryReason(analysis: any): string {
    if (analysis.hasTechnicalIssues) return 'technical_failure';
    if (analysis.hasCanonViolations) return 'canon_violation';
    if (analysis.qualityScore < 0.8) return 'quality_improvement';
    return 'general_retry';
  }

  private selectRetryStrategy(analysis: any): 'immediate' | 'delayed' | 'parameter_adjustment' | 'fallback' {
    if (analysis.hasTechnicalIssues) return 'delayed';
    if (analysis.hasCanonViolations) return 'parameter_adjustment';
    if (analysis.qualityScore < 0.6) return 'fallback';
    return 'delayed';
  }

  private generateFallbackConfigBase(analysis: any): any {
    return {
      strategy: 'conservative_generation',
      parameters: {
        steps: 20,
  ...
  private calculateRetrySuccessProbability(analysis: any): number {
    let probability = 0.8; // Base probability
    
    if (analysis.hasTechnicalIssues) probability -= 0.2;
    if (analysis.hasCanonViolations) probability -= 0.3;
    if (analysis.qualityScore < 0.6) probability -= 0.2;
    if (analysis.warningsCount > 2) probability -= 0.1;
