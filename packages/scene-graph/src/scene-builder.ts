...
export class DefaultSceneBuilder {
  ...
  private getVisibleCharacters(scene: Scene, camera: SceneCamera): SceneCharacter[] {
  ...
  private buildEnvironmentPrompt(environment: SceneEnvironment): string {
    const parts: string[] = []
  ...
  private buildCharacterPrompt(characters: SceneCharacter[]): string {
    if (characters.length === 0) {
      return ""
    }
  ...
  private buildCameraPrompt(camera: SceneCamera): string {
    const parts: string[] = []
  ...
  private buildLightingPrompt(lighting: SceneLighting): string {
    const parts: string[] = []
  ...
  private buildStylePrompt(metadata: Scene["metadata"]): string {
    const parts: string[] = []
