...
export class DefaultSceneGraph {
    scenes = new Map();
    ...
    async updateScene(sceneId, changes) {
    ...
    async resolveSceneAssets(scene) {
        const assets = [];
    ...
    applyFilter(scenes, filter) {
        return scenes.filter(scene => {
            ...
            if (filter.categories && filter.categories.length > 0) {
                const hasAllCategories = filter.categories.every(category => scene.metadata.categories.includes(category));
                if (!hasAllCategories) {
                    return false;
                }
            }
...
export function cloneScene(scene, newTitle) {
    const now = new Date().toISOString();
