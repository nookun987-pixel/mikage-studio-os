...
export function addChildNode(parent, child) {
...
export function removeChildNode(parent, childNodeId) {
    return {
        ...parent,
...
export function findNodesByType(root, nodeType) {
    const nodes = [];
...
export function traverseSceneNode(root, callback) {
    callback(root);
    for (const child of root.children) {
        traverseSceneNode(child, callback);
    }
}
export function getWorldTransform(node, parentTransform) {
...
export function validateSceneNode(node) {
    const errors = [];
    if (!node.nodeId) {
