...
export class CanonRegistryValidator {
    registryPath;
    registry = null;
    constructor(configsPath = 'configs') {
    ...
    async validateBasicStructure(issues, warnings) {
        if (!this.registry)
            return;
    ...
    async validateAuthorityChain(issues, warnings) {
        if (!this.registry)
            return;
        ...
        const actualLevels = chain.levels.map((l) => l.level).sort((a, b) => a - b);
    ...
    async validateCanonicalSources(issues, warnings) {
        if (!this.registry)
            return;
        ...
        for (const [category, categorySources] of Object.entries(sources)) {
            for (const [sourceName, source] of Object.entries(categorySources)) {
    ...
    async validateFileExistence(issues, warnings) {
        if (!this.registry)
            return;
    ...
    async validateHierarchyIntegrity(issues, warnings) {
        if (!this.registry)
            return;
    ...
    async validatePromptCanon(prompt, issues, warnings) {
        // Check mode is valid
    ...
    async validateVisualAuthority(config, issues, warnings) {
...
//# sourceMappingURL=canon-registry-validator.js.map