...
const compileRequest = compileRequestSchema.parse({
  requestCode: 'audit_compile_smoke_001',
  projectSlug: 'mikage',
...
const validationResult = validateCanon({
  requestCode: 'audit_validation_smoke_001',
  queryMode: 'blocking',
...
const productionPackage = assembleProductionPackage(
  productionPackageInputSchema.parse({
    requestCode: 'audit_package_request_001',
    packageCode: 'pkg_audit_001',
    packageMode: 'production_with_audit_placeholder',
    job: {
      jobCode: 'job_audit_001',
      projectSlug: 'mikage',
      presetCode: 'mikage_cinematic_portrait',
      variantCode: 'storm_rooftop_action',
    ...
    benchmarkAudit: {
      ...
      status: 'pending',
      notes: ['Benchmark audit not implemented before Layer 7.'],
      metadata: {}
    },
    metadata: {
      source: 'smoke-layer7'
...
const auditInput = benchmarkAuditInputSchema.parse({
  requestCode: 'benchmark_audit_smoke_001',
  packageReference: {
    packetRef: {
      packetKind: 'production_package',
      packetCode: 'pkg_audit_001',
      packetVersion: 1
    },
    productionPackage
  },
...
const result = auditBenchmark(auditInput);
  ...
  JSON.stringify(
    {
      ...
      similarityScores: result.similarityScores.map((item) => ({
        label: item.label,
        score: item.score
      })),
      driftScore: result.driftScore.score,
