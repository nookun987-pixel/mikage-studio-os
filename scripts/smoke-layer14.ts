import { executeStudioApplication } from '../packages/studio-application-facade/src/index.ts';
import { studioFacadeRequestSchema } from '../packages/studio-application-facade/src/contracts.ts';
import { orchestrationChainRequestShellSchema } from '../packages/runtime-orchestration-boundary/src/contracts.ts';

const buildRuntimeRequest = (requestCode: string) =>
...
const happyPath = executeStudioApplication(
  studioFacadeRequestSchema.parse({
    facadeMode: 'studio_runtime_execution',
    runtimeRequest: buildRuntimeRequest('facade_happy_001')
  })
);

const validationRejected = executeStudioApplication(
  studioFacadeRequestSchema.parse({
    facadeMode: 'studio_runtime_execution',
    runtimeRequest: orchestrationChainRequestShellSchema.parse({
      ...buildRuntimeRequest('facade_validation_reject_001'),
      validationProfile: {
        ...buildRuntimeRequest('facade_validation_reject_001').validationProfile,
...
console.log(
  JSON.stringify(
    {
      happyPath: happyPath.summary,
      validationRejected: validationRejected.summary
    },
    null,
    2
  )
