import { runCliEntry } from '../packages/runtime-cli-entry/src/index.ts';
import { cliEntryInputSchema } from '../packages/runtime-cli-entry/src/contracts.ts';
...
const happyPath = runCliEntry(
  cliEntryInputSchema.parse({
    entryMode: 'runtime_cli_input',
    runtimeRequest: buildRuntimeRequest('cli_happy_001')
  })
);

const validationRejected = runCliEntry(
  cliEntryInputSchema.parse({
    entryMode: 'runtime_cli_input',
    runtimeRequest: orchestrationChainRequestShellSchema.parse({
      ...buildRuntimeRequest('cli_validation_reject_001'),
      validationProfile: {
        ...buildRuntimeRequest('cli_validation_reject_001').validationProfile,
