import { normalizePlatformError } from '../packages/error-taxonomy-hardening/src/index.ts';
import {
...
const main = async () => {
  const runtime = await createIsolatedRuntimeServiceHost('layer47', 43147);
  ...
  try {
    ...
    const storageError = normalizePlatformError({
      surface: 'storage',
    ...
    console.log(
      JSON.stringify(
        {
          surfaceCodes: surface.items.map((item) => item.code),
          storageCode: storageError.code,
          storageSeverity: storageError.severity
        },
        null,
        2
      )
